...
# Project root is one level above this file's directory (server/)
...
def clean_llm_json(raw_text: str) -> dict:
    ...
    if brace_start != -1:
        ...
        text = text[brace_start : brace_end + 1] if brace_end > brace_start else text[brace_start:]#Copyright (c) 2026 Tanay Kumar Singh (@Tanay-Singh07). All Rights Reserved
...
_ui_dir = str(_PROJECT_ROOT / "dist")
...
# ---------------------------------------------------------------------------
# CLI ENTRY POINT — called by `[project.scripts] server = "server.app:main"`
# ---------------------------------------------------------------------------

def main() -> None:
    """Launch uvicorn when invoked via the `server` console script."""
    import uvicorn
